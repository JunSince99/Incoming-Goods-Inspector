...
class _AutoCameraPageState extends State<AutoCameraPage> {
  ...
  Future<void> fetchBarcodes() async {
    ...
    for (String pdc in fetchedProductCodes) {
      List<String>? barcodes = await firebaseDatabase.getBarcodesByProductCode(pdc);
                    ...
                    SizedBox(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          int originalIndex = fetchedProductCodes.indexOf(filteredProducts[index]);
                          return Card(
                            color: Colors.white,
                              ...
                              onTap: () {
                                ...
                                showDialog(
                                  ...
                                  builder: (context) => AlertDialog(
                                    title: Text('${fetchedProductNames[originalIndex]} 상품에 바코드를 추가합니다'),
                                    actions: [
                                      TextButton(
                                        onPressed: () {
                                      ...
                                      TextButton(
                                        onPressed: () {
                                          addBarcodeToProduct(barcode.rawValue.toString(),fetchedProductCodes[originalIndex]);
                                          Navigator.of(context).pop();
                                          Navigator.of(context).pop();
                                        },
                                        child: const Text('확인'),
  ...
  Future<void> addNewProductInDatabase() async {
    Map<String, dynamic>? barcodeProductCodeData = await FirebaseDatabase().getDatabaseDoc("productcode_productname");

    if (barcodeProductCodeData != null && barcodeProductCodeData != {}) {
      for(String pdc in fetchedProductCodes){
        if(!barcodeProductCodeData.containsKey(pdc)) {
          FirebaseDatabase().insert("productcode_productname", pdc, fetchedProductNames[fetchedProductCodes.indexOf(pdc)]);
        }
      }
    }
  }
  ...
  Future<void> addBarcodeToProduct(String newbcd, String pdtcode) async {
    FirebaseDatabase().insert("barcode_productcode", newbcd, pdtcode);
    fetchBarcodes();
  }
  ...
  Future<void> getSampleProductList() async {
    setState(() {
      fetchedProductCodes = ["7026118","7026120","8007043","8020728","8025917","8047438","8048765","8050553","8053496","7026814","7030269","7044666","8017429","8018135","8023693","8028150","8028151","8037415","8048883","8051831","8052363","7012711","7015208","7044940","8025458","8042535","7047248","8020494","8051345","7040118","8050892"];
      fetchedProductNames = ["코카)글라소파워씨500ML","코카)글라소에너지500ML","코카)모과생강280ML","코카)토레타500ML","코카)코카콜라제로250ML","코카)갈아만든배340ml","코카)몬스터에너지제로슈거355ML","코카)조지아저칼로리라떼470ML","코카)몬스터에너지피치킨355ML","롯데)크런키","롯데)목캔디파워허브","롯데)빈츠","롯데)빅크런키","롯데)후라보노","롯데)자일리톨알파(용기)","롯데)청포도캔디","롯데)애니타임밀크민트","롯데)왓따청포도","롯데)제로크런치초코볼","롯데)졸음번쩍오리지널(코팅)","롯데)제로쿠앤크샌드","광동)비타500(100ML)","광동)옥수수수염차340ML","광동)제주삼다수500ML","광동)비타500젤리48g","광동)비타500 치어팩","엠큐)프링글스양파맛110g(대)","메비우스 스카이블루(곽)","메비우스 스카이블루 롱스","진주)천하장사50g","진주)건강하닭할라피뇨치즈"];
      fetchedProductQuantities = ["12","12","6","24","30","6","6","6","24","12","12","2","8","90","6","2","4","16","8","12","2","100","20","40","10","6","1","50","20","16","10"];
      isChecked = List<bool>.filled(fetchedProductCodes.length, false);
      filteredProducts = fetchedProductCodes;
      fetchBarcodes();
    });
                            ...
                            SizedBox(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(fetchedProductCodes[index],style: const TextStyle(color: Colors.grey))
                                ],
                              ),
                            ),
                            Row(
                              ...
                              children: [
                                ...
                                isChecked[index]? OutlinedButton(
                                  onPressed: (){
                                    setState(() {
                                      isChecked[index] = !isChecked[index];
                                    });
